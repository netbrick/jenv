...
if [[ $(whoami) == 'root' ]]; then
	...
	if [[ -z `grep "jenv-init.sh" "/etc/profile.d/jenv.sh"` ]]; then
		jenv_profile_global=$( echo '[[ -z `grep "jenv-init.sh" "${HOME}/.profile"` ]] && export JENV_GLOBAL="true" && source "${JENV_DIR}/bin/jenv-init.sh" && source "${JENV_DIR}/commands/completion.sh"' )
